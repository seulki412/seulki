	...
	protected void doprocess(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		switch (requestURI) {
			...
			case "/admin/AdminRoomList.ad":
				...
				break;
			// 예약취소내역 리스트 
			case "/admin/AdminReservationCancelList.ad":
				forward = new AdminReservationCancelListAction().execute(req, resp);
				break;
...
//------------------------------------------------------------------------------------------------				
			...
			case "/admin/AdminRoomView.ad":
				...
				break;
			// 객실 추가 페이지 받기
			case "/admin/AdminRoomAdd.ad": 
				forward = new AdminRoomAddAction().execute(req, resp);
				break;
			// 객실 추가
