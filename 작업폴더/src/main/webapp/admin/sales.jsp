...
<body class="sb-nav-fixed">
	...
	<c:set var="salesList" value="${requestScope.salesList }" />
							...
							<div class="collapse" id="collapsePages"
								...
								<nav class="sb-sidenav-menu-nested nav accordion"
									...
									<a class="nav-link collapsed"
										...
										회원정보 </a> <a class="nav-link collapsed"
										href="${pageContext.request.contextPath }/admin/AdminSalesList.ad">
										매출정보 </a> <a class="nav-link collapsed"
						...
						<div class="card-body">
							<table id="datatablesSimple">
								...
								<tbody>
									...
									<c:choose>
										<c:when
											test="${salesList != null and fn:length(salesList) > 0 }">
											<c:forEach var="sales" items="${salesList }">
												<tr>
													<td>${sales.res_number}</td>
													<td>${sales.res_rtype}</td>
													<td>${sales.res_price}</td>
												</tr>
											</c:forEach>
										</c:when>
								...
								<c:forEach var="result" items="${salesList}" varStatus="status">
									<c:set var="total" value="${total + result.res_price}" />
								</c:forEach>
								<th>
									<c:out value="${total}" />
								</th>
